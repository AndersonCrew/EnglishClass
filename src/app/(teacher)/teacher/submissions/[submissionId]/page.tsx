...
export default async function SubmissionResultPage({ params }: { params: Promise<{ submissionId: string }> }) {
  ...
  return <main className="mx-auto max-w-4xl px-5 py-8 sm:px-8">
    ...
    <section className="mt-7"><h2 className="text-2xl font-bold">Speaking · Giáo viên chấm</h2><p className="mt-1 text-slate-500">Nghe và chấm điểm riêng từng câu, sau đó lưu toàn bộ kết quả.</p><div className="mt-4 space-y-4">{speaking.map((answer, index) => <article key={answer.id} className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200"><div className="flex gap-4">{answer.question.image_url && <Image unoptimized src={answer.question.image_url} width={112} height={84} alt="Hình minh họa câu Speaking" className="h-20 w-28 shrink-0 rounded-xl object-cover ring-1 ring-slate-200" />}<div><p className="text-sm font-bold text-violet-700">Câu Speaking {index + 1}</p><h3 className="mt-2 text-lg font-bold">{answer.question.prompt}</h3></div></div>{answer.audio_url ? <audio controls preload="metadata" src={answer.audio_url} className="mt-4 w-full" /> : <p className="mt-4 rounded-xl bg-rose-50 p-3 text-sm font-semibold text-rose-700">Không tìm thấy bản thu âm.</p>}</article>)}</div><SpeakingAssessmentForm answers={speaking.map((answer) => ({ id: answer.id, prompt: answer.question.prompt, teacherScore: answer.teacher_score, teacherFeedback: answer.teacher_feedback }))} /></section>

    <section className="mt-7"><h2 className="text-2xl font-bold">Listening, Reading và Writing</h2><p className="mt-1 text-slate-500">Đáp án học sinh, đáp án đúng, hình minh họa và điểm từng câu.</p><div className="mt-4 space-y-4">{objective.map((answer, index) => <article key={answer.id} className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-xs font-black text-teal-700">{answer.question.task?.skill} · CÂU {index + 1}</p><span className={`rounded-full px-3 py-1 text-xs font-bold ${answer.is_correct ? "bg-emerald-100 text-emerald-700" : "bg-rose-100 text-rose-700"}`}>{answer.is_correct ? "Đúng" : "Chưa đúng"} · {answer.auto_score ?? 0}/{answer.question.points}</span></div><div className="mt-3 flex gap-4">{answer.question.image_url && <Image unoptimized src={answer.question.image_url} width={112} height={84} alt="Hình minh họa câu hỏi" className="h-20 w-28 shrink-0 rounded-xl object-cover ring-1 ring-slate-200" />}<h3 className="font-bold">{answer.question.prompt}</h3></div><div className="mt-4 grid gap-3 sm:grid-cols-2"><AnswerBox label="Học sinh trả lời" value={formatAnswer(answer.answer, answer.question.config)} tone="student" /><AnswerBox label="Đáp án đúng" value={formatAnswer(answer.question.answer_key, answer.question.config)} tone="correct" /></div></article>)}</div></section>
  </main>;
}
