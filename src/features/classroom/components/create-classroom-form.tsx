...
export function CreateClassroomForm() {
  ...
  return <form action={action} className="mt-7 space-y-5 rounded-2xl bg-white/90 p-6 shadow-md shadow-teal-100/60 ring-1 ring-teal-100 sm:p-8">
    ...
    <button disabled={pending} className="flex w-full items-center justify-center gap-2 rounded-xl bg-teal-600 px-5 py-3 font-bold text-white disabled:opacity-60">{pending && <span className="size-4 animate-spin rounded-full border-2 border-white/40 border-t-white" />}{pending ? "Đang tạo và mở lớp..." : "Tạo lớp"}</button>
  </form>;
}
