...
function defaultCloseTime() {
  const date = new Date(Date.now() + 24 * 60 * 60 * 1000 + 5 * 60 * 1000);
...
export function AssignmentStatusActions({ assignmentId, classroomId, locked }: { assignmentId: string; classroomId: string; locked: boolean }) {
  const router = useRouter(); const [minimumTime] = useState(() => { const date = new Date(Date.now() + 24 * 60 * 60 * 1000 + 60_000); const offset = date.getTimezoneOffset() * 60_000; return new Date(date.getTime() - offset).toISOString().slice(0, 16); });
  ...
  const run = () => {
    ...
    if (!closesAt || Number.isNaN(closeTime.getTime()) || closeTime.getTime() < Date.now() + 24 * 60 * 60 * 1000) {
      setMessage("Thời gian kết thúc phải cách thời gian mở bài ít nhất 24 giờ.");
      return;
    }
    startTransition(async () => { const result = await openAssignmentUntilAction(assignmentId, classroomId, closesAt); setMessage(result.message); if (result.ok) { setDialogOpen(false); router.refresh(); } });
  };
  return <>
    ...
    {dialogOpen && createPortal(<div className="fixed inset-0 z-[100] grid place-items-center bg-slate-950/40 p-5 backdrop-blur-sm" onMouseDown={(event) => { if (event.target === event.currentTarget && !pending) setDialogOpen(false); }}>
      <div role="dialog" aria-modal="true" className="w-full max-w-md rounded-2xl bg-white p-6 text-left shadow-2xl">
        ...
        <p className="mt-4 leading-7 text-slate-600">Bài sẽ mở ngay và tự động khóa khi hết thời gian. Thời gian làm bài tối thiểu là 24 giờ và giáo viên không thể đóng thủ công.</p>
